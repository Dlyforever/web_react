...
class SettingCreateTopic extends React.Component {
    ...
    onModelOk(e){
        ...
        if(objectValue === "") {
            message.warning('主题词不能为空！');
            return;            
        }else if (objectValue.length >=300){
            message.warning('主题词请不要超过300个字符！');
            return;   
        }else if (subject1Value.length >=200 || subject2Value.length >=200){
            message.warning('关联词请不要超过200个字符！');
            return;   
        }else if (filterValue.length >=200){
            message.warning('排除词请不要超过200个字符！');
            return;   
        }
