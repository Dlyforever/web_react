...
class ModalAllOpinion extends React.Component{
    ...
    trendClick(value){
        this.setState({
         ...
         let newSeltype = isSearch? searchSeltype : seltype;
         let newKeyword = isSearch? searchKeyword : keyword;
         const requestStr =topicId!==undefined?`topicid=${topicId}&datetag=custom&neg=${value}&order=${sortValue}&similer=${filterValue}&carry=${mediaValue}&begin=${startDate}&end=${endDate}&page=1&seltype=${newSeltype}&keyword=${newKeyword}`: startDate?`datetag=custom&neg=${value}&order=${sortValue}&similer=${filterValue}&carry=${mediaValue}&begin=${startDate}&end=${endDate}&page=1&seltype=${newSeltype}&keyword=${newKeyword}`:`datetag=${timeValue}&neg=${value}&order=${sortValue}&similer=${filterValue}&carry=${mediaValue}&begin=${begin}&end=${end}&page=1&seltype=${newSeltype}&keyword=${newKeyword}`
         request(api_total_opinion,{
          method: 'POST',
          headers: {
                "Content-Type": "application/x-www-form-urlencoded"
    ...
    checkItem(sids){
     ...
     keyDown = (e) => {
        ...
        if(code === 13){
            ...
            startDate?`begin=${startDate}&end=${endDate}&datetag=custom&neg=all&order=timedown&similer=1&carry=全部&page=1&seltype=${searchSeltype}&keyword=${e.target.value}`
