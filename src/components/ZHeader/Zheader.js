...
import { Menu, Dropdown, Modal, Avatar,message} from 'antd';
import {setItem, getItem} from '../../utils/localStorage';
import {history} from '../../utils/history';
import request from '../../utils/request';
import {api_logout,api_revise_userinfo,api_get_userinfo,api_homepage_message,api_sub_sys} from '../../services/api';
import './ZHeader.less';
...
class Zheader extends React.Component {
    ...
    handleUserInfoOk() { 
        ...
        request(api_get_userinfo).then(res=>{
            this.setState({
                id: res.data.id,
                oldPassword:userMessage.password
            })
        ...
        //   }
              ...
              request(api_revise_userinfo,{
                 ...
                 body:`&oldPass=${this.state.oldPassword}&id=${this.state.id}&nickname=${userMessage.nickname}&password=${userMessage.newpassword}&tel=${userMessage.tel}&email=${userMessage.email}`
             }).then(res=>{
                 if(res.data.code===1){
    ...
    logoutSystem(data) {
        ...
        confirm({
            ...
            onOk() {
                ...
                window.sessionStorage.setItem('logoinfo', '{}')
            },
            onCancel() {
                message.info('您取消了操作！');
            },
            ...
            <div className="z-header" style={{backgroundColor:themeColor.topColor.backgroundColor}}>
                <div className="top">
                    <div className="left" onClick={this.goBackIndex.bind(this)}>
                         <img src={ this.state.logoinfo.logoimg ? this.state.logoinfo.logoimg : logo} style={{width: 100, height: 32}} alt="logo" className="logo" onClick={this.goBackIndex.bind(this)}/>
                        
                          {/* <img src={logo} alt="logo" className="logo" onClick={this.goBackIndex.bind(this)}/> */}
                         <span className="name" style={{color:'#fff', marginLeft: 10}}>{ this.state.logoinfo.userSysName}</span>
                    </div>
                    <div className="right">
