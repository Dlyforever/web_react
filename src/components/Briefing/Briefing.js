...
export class Briefing extends React.Component {
            ...
            if(!nexprops.visible) {
                let timer = setInterval(() => {
                    ...
                    if(document.querySelector('.briefing').offsetHeight <= 20) {
                        ...
                        document.querySelector('.briefing').style.display = 'none'
                        clearInterval(timer)
                    }
                }, 10)
            }else {
    ...
    statusicon(status) {
        switch (status) {
            case '0':
                return <IconFont type="icon-weiwancheng" style={{width: 18, height: 18}}></IconFont>
            case '1':
                return <IconFont type="icon-wenhao" style={{width: 16, height: 16}}></IconFont>
            case '2':
                return <IconFont type="icon-duihao" style={{width: 18, height: 18}}></IconFont>
            default:
            break;    
        }
    }
        ...
        const reportList = () => {
            return this.state.report.length === 0 ? <h4>没有数据</h4> : this.state.report.map((item, index) =>
                    <li key={index} data-id={item.id} onDragEnter={this.ondragenter.bind(this)} onDragOver={this.ondragover.bind(this)} onDragLeave={this.ondragleave.bind(this)} onDrop={this.ondrop.bind(this)} onDragEnd={this.ondragend.bind(this)}>
                        ...
                        <div className="des">
                            <div className="text">
                                <span className="name">{item.reportname.length > 5 ? item.reportname.slice(0, 5) + '...' : item.reportname}</span>
                                <span className="date">{typeof  item.updateTime === 'number' ? this.timestampToTime(item.updateTime) : item.updateTime}</span>
                            </div>
