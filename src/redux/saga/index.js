...
// 搜索弹出舆情
function* getyvqingList(action) {
    try {
        const result = yield call(apiSearchejctYuqin, action.payload);
        yield put(yvqingListSucceeded(result));
    } catch (e) {
        console.log(e)
    }
}
function* getyvqingListSaga() {
    yield takeLatest("GET_SEARCHEJCT_YUQING_REQUESTED", getyvqingList);
}

// 搜索弹出分类
function* getclassifyList(action) {
    try {
        const result = yield call(apiSearchejctClassify, action.payload);
        yield put(classifySucceeded(result));
    } catch (e) {
        console.log(e)
    }
}
function* getclassifyListSaga() {
    yield takeLatest("GET_SEARCHEJCT_CLASSIFY_REQUESTED", getclassifyList);
}

// 搜索弹出专题
function* getspecialList(action) {
    try {
        const result = yield call(apiSearchejctSpecial, action.payload);
        yield put(specialSucceeded(result));
    } catch (e) {
        console.log(e)
    }
}
function* getspecialListSaga() {
    yield takeLatest("GET_SEARCHEJCT_SPECIAL_REQUESTED", getspecialList);
}

//取证路由信息
function* getEvideinfo(action) {
    try {
        const result = yield call(apiEvideinfo, action.payload);
        yield put(evideinfoSucceeded(result));
    } catch (e) {
        console.log(e)
    }
}
function* getEvideinfoSaga() {
    yield takeLatest("GET_EVIDEINFO_REQUESTED", getEvideinfo);
}
export default function* root() {
    yield all([
        ...
        fork(getyvqingListSaga),
        fork(getclassifyListSaga),
        fork(getspecialListSaga),
        fork(getEvideinfoSaga)
    ])
}