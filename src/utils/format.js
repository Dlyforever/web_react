...
export function setHighlightTags(content, tags) {
   const tagLength = tags.length;
    ...
    if (tagLength > 0  ) {
        tags.forEach(item => {    
            if(content) {
                result = result.replace(new RegExp(item, 'g'),`<span style="color: red;">${item}</span>`);
            }
        })
    } else {
        result = content;
...
export function urlTokey() {
    ...
    //     url = hash.match(/^(#\/)(\w+)/);
    // }
    url = hash.split('/').length > 4 ? hash.split('/')[2] + hash.split('/')[3] : hash.split('/')[2] 
    let key = '0';
    switch (url)
    {
        case 'home':
        ...
        case 'evidadmin':
            key = 'evidadmin'
            break; 
        case 'evidencemanagementinterent':
            key = 'interent' 
            break;                
        default:
            break;
...
//首页自定义
export function customHome(data){
    let list = [];
    if(data.length < 1) {
        return []
    }
    if(data[0]['defaultSize'] === 33.3){
        list = data.splice(0,3);
    }else if (data[0]['defaultSize'] === 40 || data[0]['defaultSize'] === 50 || data[0]['defaultSize'] === 60){
        list = data.splice(0,2);
    }else{
        list = data.splice(0,1);
    }
    return [list].concat(customHome(data))
}
//首页自定义判断重复
export function isRepeat(data,newData){ 
    let arr = [];
    let newArr = [];
    let list = [];
    data.forEach(item => {
