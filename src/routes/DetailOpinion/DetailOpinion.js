...
class DetailOpinion extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.num !== this.state.num) {
      ...
      let str = pathname.length === 3 ? `&sid=${pathname[2]}` : `&sid=${pathname[2]}&type=${pathname[3]}`
      request(api_get_doc_detail + str).then((res) => {
        ...
        request(api_get_doc_similar + '&sid=' + pathname[2], {}).then((res) => {
  ...
  showEditModal() {
    this.setState({
      editModalVisible: true
       ...
       this.setState({
        isChange: this.state.isChange +1,
       })
    })
  }
  ...
  editModalHandleOk() {
    ...
    request(api_docedit_save, {
      ...
      body: `author=${editData.author}&sid=${editData.sid}&title=${editData.title}&pubdate=${editData.pubdate}&timestamp=${editData.timestamp}&source=${editData.source}&url=${editData.url}&clickcount=${editData.clickcount}&replycount=${editData.replycount}&hot=${editData.hot}&nrtags=${editData.nrtags}&keyword=${editData.keyword}&match=${editData.match}&carry=${editData.carry}&negative=${editData.negative}&summary=${editData.summary}&content=${encodeURIComponent(editData.content)}&reloadtag=${editData.reloadtag}`
    }).then(res => {
                ...
                <div className="wrapper">
                    ...
                    <div className="modals">
                        <Modal title="修改"
                               ...
                               style={{marginLeft:'30%'}}
                        >
                            <EditOpinionDetail titleSid={sid} isChange={this.state.isChange}/>
                        </Modal>
                        {/* <Modal
                          title="推送消息"
