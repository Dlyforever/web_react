...
import {evideinfoRequested} from "../../redux/actions/createActions";
class EvideInfo extends React.Component {
  ...
  componentWillMount(){
      this.props.evideinfoRequested(`&fileName=${this.props.location.search.split('=')[1]}`)
  }
  urlToObj(url) {
    let obj = {}
  ...
  render() {
    ...
    return (
      <div className="evide-info">
        <div className="evide-top">
        <div className="evide-title" title={this.props.routerInfo.title}>{this.props.routerInfo.title}</div>
        <Menu
...
const mapDispatchToProps = dispatch => {
  return {
    evideinfoRequested: req => {
      dispatch(evideinfoRequested(req));
    }
  }
};
export default connect(mapStateToProps,mapDispatchToProps)(EvideInfo);