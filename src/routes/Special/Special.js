...
class Special extends React.Component{
    ...
    request(api_update_report + '&reportId=' + this.state.reportId + '&reportTitle=' + e + '&moduleId=' + this.state.dataID).then((res) => {
			if(res.data.code === 1) {
				...
				jiaData.data[componentId[0]]['reportTitle'] = e;
				this.setState({
					jiaData:jiaData    
				})
			} else {
	...
	onChangeCellEditor(e) {
		...
		request(api_update_report + '&reportId=' + this.state.reportId + '&editor=' + e + '&moduleId=' + this.state.dataID).then((res) => {
			if(res.data.code === 1) {
				message.success(res.data.msg);
				jiaData.data[componentId[0]]['editor'] = e;
				this.setState({
					jiaData:jiaData    
				})
			} else {
	...
	onChangeCellDate(e) {
		...
		request(api_update_report + '&reportId=' + this.state.reportId + '&date=' + e + '&moduleId=' + this.state.dataID).then((res) => {
			if(res.data.code === 1) {
				message.success(res.data.msg);
				jiaData.data[componentId[0]]['date'] = e;
				this.setState({
					jiaData:jiaData    
				})
			} else {
			...
			<div className="col">
				...
				> 
				  <ModalReport
				   ...
				   topicid={this.state.topicid}
				   startDate={this.state.jiaData.starttime}
				   endDate={this.state.jiaData.endtime}
				  />
				</Modal>
			</div>
		)
