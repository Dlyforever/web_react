...
class BiddingSetting extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            seniorWord: ['匹配关键词', '정합 키워드.', 'キーワードに合う', 'ئاچقۇچلۇق سۆز ماس كېلىدىغان ', 'ཟླ་སྒྲིག་གནད་ཚིག་', 'Matching keyword combination'],
            seniorWordTip: {
            ...
            save: ['确认', '확인', '確認', 'بېكىتىش ', 'ཁས་ལེན་གསལ་ཐག་ཆོད་པ་', 'confirm'],
            ...
            topiccatrepeat: ['该话题已存在分组中', '이 화제는 이미 조 편성 되었다.', 'この問題は既にパケットに存在する。', 'بۇ تېما ئاللىقاچان مەۋجۇت گۇرۇپپا', 'བརྗོད་གཞི་དེ་གནས་པའི་ཚོ་བགོད་ཁྲོད་', 'This topic is already in the group']
        }
    }
    ...
    componentWillMount(){
        if (this.search2Obj(this.props.location.search).type === 'add') {
            ...
            request(api_sorted_cat_list + `&lang=${this.state.language[this.props.match.params.languages]}`).then((res) => {
                ...
                request(api_sorted_rule_list +'&clfid=' +topicid + '&lang=' + this.state.language[this.props.match.params.languages]).then(res2=>{
    ...
    handleSubmit(e) {
        ...
        //     }               
        // });
        if(this.state.topicNameValue.length>=28){
        ...
        if( this.search2Obj(this.props.location.search).type === 'add' ) {
            request(api_sorted_rule_add, {
                ...
                if(res.data.code===1){
                    ...
                    history.push('/allopinion/multilingual/'+ this.props.match.params.languages +'/multilingual')
                }else {
                    message.error(this.state.topiccatrepeat[this.props.languages])
                }
            })
        } else {
