...
class OpinionDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      negText: {
        ...
        warning: ['预警', '예보', '警報', 'ئالدىن سىگنال بېرىش ', 'ཉེ་བརྡ་', 'warning']
      },
      ...
      handleTip: {
        ...
        warningsuccess: ['成功设置为预警', '성공', '成功', 'مۇۋەپپەقىيەت', 'གྲུབ་འབྲས་ཐོབ་པའི་ངང་སྔོན་བརྡའི་ཆེད་འཇོག་', 'Successfully set an warning'],
      },
  ...
  handleOk() {
    ...
    request(api_allopinion_exportskip, {
      ...
      body: `page=${propsParamData.page}&clfid=${this.props.getRouterReducer.topicid}&sids=${propsParamData.sid}&lang=${this.props.lang}&pagesize=${propsParamData.pagesize}&datetag=${propsParamData.datetag}&taskname=${this.state.fileName}&documenttype=excel&createdate=${time}&taskstate=0&source=monitor&order=${propsParamData.order}&begin=${propsParamData.begin}&end=${propsParamData.end}&neg=${propsParamData.neg}&carry=${propsParamData.carry}&similer=${propsParamData.similer}&seltype=content&keyword=`
    }).then(res => {
          this.setState({
            downloadFlag:false
          })
  ...
  render() {
    ...
    // debugger
    const itemLeft = (item) => {
      if(parseInt(this.props.languageType,10) === 3) {
        ...
        <div className="key">
          ...
          <div className="similar-info">
            <div className="similar-right">
              <span>{this.state.infoCompany[this.props.languageType]}</span> <span style={{float: 'right'}}>{item.similerInfo && (item.similerInfo.similerCount ? item.similerInfo.similerCount : 0)}</span>
            </div>
          ...
          <div className="title" style={{marginRight: 20}}>：{this.state.keyWord[this.props.languageType]}</div>
          <div className={this.state.isSearch ? '' : 'keywords' } style={{minWidth: 0}}>
            {item.nztags}
          </div>
        </div>
      </div>
    ...
    const OpinionDetailItems = docList[0] !== undefined && docList[0]['negative'] !== undefined ? docList.map((item, index) =>
        <li key={item.sid} className={parseInt(this.props.languageType,10) === 3 ? 'uyguritem opinion-detail-item' : 'opinion-detail-item'}>
          ...
          <div className="iconBox">
            <div className="negative">
              <div className="inner-type" style={opinionColor(item.negative)} title={this.opinionTrend(item.negative)}>
      ...
      <BlankPage desc={this.props.propsType === 'TopicList' ?
        `<span>空空如也，赶紧去<a href="index.html#/allopinion/multilingual/${this.props.langnum}/setting?topicid=${this.props.topicid}">添加</a>关键词</span>` :
        `<span>空空如也，赶紧去<a href="index.html#/allopinion/multilingual/${this.props.langnum}/setting?topicid=${this.props.topicid}">添加</a>关键词</span>`
      }/>
    ;
    ...
    const left = () => {
      if(this.props.languageType-0 === 3) {
        return <div className="left" style={{marginRight: 3}}>
    ...
    return (
      <div className="mul-detail">
        ...
        >
          ...
          ></Input>
          <Button type="primary"
                  style={{margin: '20px 0 0 214px', height: '30px'}}
