...
import {Menu,Modal,Input,Dropdown,message, Button, Icon} from 'antd';
...
class BiddingOpinion extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            subjectName: ['请输入主题名称', '제목을 입력하십시오.', '件名を入力してください', 'تېمىنى كىرگۈزۈڭ نامى ', 'བརྗོད་བྱ་གཙོ་བོའི་མིང་འབྲི་རོགས་།', 'Please enter a subject name'],
            ...
            yesIknow: ['知道了', '알겠습니다', '知った', 'بولىدۇ', 'ངས་ཤེས་སོང་', 'Know the'],
            catrepeat: ['该文件夹已存在', '이 파일은 이미 존재하였다.', 'このフォルダは既に存在している。', 'بۇ ھۆججەت قىسقۇچ مەۋجۇت', 'ཡིག་ཁུག་དེ་གནས་', 'This folder already exists']
        };
    ...
    componentWillReceiveProps(nextprops){
        ...
        if(nextprops.location.pathname.split('/')[4] === 'multilingual' &&  nextprops.location.pathname !== this.props.location.pathname && this.props.match.params.languages !== nextprops.match.params.languages){
            let get = () =>{
    ...
    handleOk(e){
        ...
        if(this.state.inputValue.length>=28){
        ...
        .then((res) => {
            if(res.data.code !== 1) {
                message.error(this.state.catrepeat[this.props.languages])
                return false;
            }
            this.initCard()
        })
         ...
         request(api_sorted_grade_delete,{
              ...
              body:`clfid=${this.state.topicId}&lang=${this.state.language[this.props.match.params.languages]}`
                ...
                if(res.data && res.data.code===1){
                    if(this.props.location.pathname === `/allopinion/multilingual/${this.props.match.params.languages}/setting`) {
                        history.push({
                            pathname:`/allopinion/multilingual/2/multilingual`
                        });
                    }else{
                        let get = () =>{
                            let topicMessage = this.state.topicNavMessage;
                        ...
                        this.initCard(this.props.match.params.languages, get)
                    }
                    message.success(this.state.delsuccess[this.props.languages])
                }
                this.initCard()
   ...
   ondeltopiccan() {
    ...
    render() {
        const delItems = (item, index) => {
            return <Menu onClick={this.onDelitem.bind(this,item.catid)}>
            {index === 0 ? '' : <Menu.Item key="2">{this.state.rename[this.props.languages]}</Menu.Item>}
            {index === 0 ? '' : <Menu.Item key="1">{this.state.del[this.props.languages]}</Menu.Item>}
        ...
        const LeftTopicLists=this.state.topicNavMessage!==1&& this.state.topicNavMessage && this.state.topicNavMessage.map((item,index)=>
          ...
          <Dropdown overlay={delItems(item, index)} trigger={['click']}>
        ...
        return (
            <div className="topic-opinion">
                ...
                <div className="left-boxes">
                    ...
                    <Modal
                        visible={this.state.deltopcvisible}
                        footer={<Button type="primary" onClick={this.ondeltopiccan.bind(this)}>{this.state.yesIknow[this.props.languages]}</Button>}
                        onCancel={this.ondeltopiccan.bind(this)}
                    >
                        <div>
                            <div style={{display: 'flex', marginBottom: 20}}>
                                <Icon type="info-circle" style={{lineHeight: 1.5}}/>
                                <h3 style={{fontSize: 16, marginLeft: 15}}>{this.state.systemPrompt[this.props.languages]}</h3>
                            </div>
                            <p>{this.state.delalltopic[this.props.languages]}</p>
                        </div>
                    </Modal>
