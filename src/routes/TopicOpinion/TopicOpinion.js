...
import Del from '../../assets/img/more.svg'; 
class TopicOpinion extends React.Component {
          ...
          if(topicMessage!==1){
            ...
            for(let i in topicMessage){
                if(topicMessage[i]['topicList'][0]!==undefined){
                     ...
                     firstTopicid.begin = topicMessage[i]['topicList'][0]['begin'];
                     firstTopicid.end = topicMessage[i]['topicList'][0]['end'];
                     break;
                }
            } 
    ...
    handleAddTopic() {
        ...
        history.push(`/allopinion/topic/addtopic?catid=${this.state.catid}`);
    }
    delTopic(topicid,e){
        this.setState({
            topicId:topicid,
    ...
    queryTopic(topicid,topicname,begin,end){
        this.setState({
            materialCurrent: topicid,
            current: 'topiclist',
            topicId:topicid
        })
          this.props.setlocationPathname({topicid:topicid,topicname:topicname,begin:begin,end:end});
          history.push({
           ...
           <ul className="topics" ref={'topicList'+item.catid}>
              {item.topicList && item.topicList.map((iitem,iindex) =>
                 ...
                 className={this.state.materialCurrent === iitem.topicid ? 'backGroundBlue' : 'a-topic'}
                  >
                  <span className="topicTitle" onClick={this.queryTopic.bind(this,iitem.topicid,iitem.topicname,iitem.begin,iitem.end)}
            ...
            <div className="topic-opinion">
             {TopicList}
                ...
                <div className="topic-info">
                    ...
                    <div className="close"  style={this.state.current==='topiclist'?{display:'flex',color:BLACK, alignItems: 'center'}:{display:'none'}}>
                        <div style={{marginRight: 30}}><Button onClick={this.createReport}>生成专报</Button></div>
                    	<span onClick={this.triggerTopShow.bind(this)}>{this.state.isTopShow ? '显示' : '隐藏'}</span>
                    	<Icon type={this.state.isTopShow ? 'down' : 'right'} style={{lineHeight: 1.5}}/>
                    </div>
                    </div>
