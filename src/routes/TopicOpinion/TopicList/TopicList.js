  ...
  handleSubmit(event) {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (getSecondTime(begin) > getSecondTime(end)) {
        ...
        return;
      }else if (getSecondTime(begin) < getSecondTime(this.props.getRouter.begin)){
        message.error('开始时间请不要小于设置的专题开始时间');
        return;
      }else if (getSecondTime(end) > getSecondTime(this.props.getRouter.end)){
        message.error('结束时间请不要大于设置的专题结束时间');
        return;
      }
