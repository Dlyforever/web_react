...
class FocusSetting extends React.Component{
    ...
    submitTagname = () => {
        ...
        if(/~|!|@|#|\$|\^|&|\*|=|\?|！|￥|-|（|）|%|【|】|\{|\}|；|;|%|,|，|。|？|《|》|\./.test(tagName)){
            ...
            return;
        }else if(tagName.replace(/(^\s*)|(\s*$)/g,'') === ''){
            message.error('标签名称请不要为空');
            return;
        }
    ...
    addWebsite () { 
        ...
        if(/~|!|@|#|\$|\^|&|\*|=|\?|！|￥|-|（|）|%|【|】|\{|\}|；|;|%|,|，|。|？|《|》|\./.test(websiteName)){
            message.error('名称中请不要带有特殊字符');
            return;
        }else if(websiteName.length>18){
            message.error('字数请不要大于18个字符');
            return;
        }else if(websiteName.replace(/(^\s*)|(\s*$)/g,'') === ''){
            message.error('网站名称请不要为空');
            return;
        }
    ...
    blurTagname = (e) => {
        ...
        if(editTagname.replace(/(^\s*)|(\s*$)/g,'') === ''){
            message.error('标签名称请不要为空');
            return;
        }else if(/~|!|@|#|\$|\^|&|\*|=|\?|！|￥|-|（|）|%|【|】|\{|\}|；|;|%|,|，|。|？|《|》|\./.test(editTagname)){
            message.error('名称中请不要带有特殊字符');
            return;
        }else if(editTagname.length>18){
            message.error('字数请不要大于18个字符');
            return;
        }
    ...
    editSource = (websiteId,source) => {
         this.setState({
            websiteId:websiteId,
            editWebsitename:source
         })
    }
    ...
    blurWebsitename = () => {
        ...
        if(editWebsitename.replace(/(^\s*)|(\s*$)/g,'') === ''){
            message.error('标签名称请不要为空');
            return;
        }else if(/~|!|@|#|\$|\^|&|\*|=|\?|！|￥|-|（|）|%|【|】|\{|\}|；|;|%|,|，|。|？|《|》|\./.test(editWebsitename)){
            message.error('名称中请不要带有特殊字符');
            return;
        }else if(editWebsitename.length>18){
            message.error('字数请不要大于18个字符');
            return;
        }
    ...
    render(){
        ...
        const tagNav = tagList.map((item,index) => {
            return <Draggable key={index} className={id === item.id ?'tag-name active':'tag-name'}
             ...
             <span onDoubleClick={this.editTagName.bind(this,item.tagName)}>{item.tagName}</span>
             }
        ...
        const listKey =isBlankPage? websiteList.map((item,index) => {
            return <div key={index} className="website-name">
                    ...
                    />: <span onDoubleClick={this.editSource.bind(this,item.websiteId,item.source)}>{item.source}</span>}<span style={{marginLeft:'10px'}}>({item.total})</span></p>
               ...
               <div className="focus-content">
                  <div className="focus-tag">
                    ...
                    <div className="tag-name" style={flag?{display:'flex'}:{display:'none'}}>
                       ...
                       onBlur = {this.tagnameBlur}
                       />
                       <span onClick={this.submitTagname} onMouseDown={this.mouseDown}>提交</span>
                    </div>
                  </div>
                  <div className="right">
                      <p className="media-setting"><span>网站列表</span><span onClick={this.showListKey}>添加网站</span></p>
                      <p className="media-setting" style={isAdd?{display:'flex'}:{display:'none'}}><Input value={websiteName} onChange={this.changeWebsite}/><span onClick={this.addWebsite.bind(this)}>提交</span></p>
                      {listKey}
                  </div>
               </div>
            </div>
